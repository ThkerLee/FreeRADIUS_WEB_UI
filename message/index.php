 ...
 * 序列号,请通过亿美销售人员获取
 */
$serialNumber = '0SDK-EBB-0130-NERNQ'; 
/**
 * 密码,请通过亿美销售人员获取
 */
$password = '182786';
...
function sendSMS($tel,$message)
{
	 ...
	 */
	$statusCode = $client->sendSMS(array($tel),$message);
	//echo "处理状态码:".$statusCode;
}
...
function setMOForwardEx()
{
	...
	$statusCode = $client->setMOForwardEx(
		array('159xxxxxxxx','159xxxxxxxx')
	);
	echo "处理状态码:".$statusCode;
} 

function sendAddMessage($tel){
	global $client;
	$login=true;$balance=true;$tel_err =true;$tel_num_err=true;
	
  if(strlen($tel) == "11" && is_numeric($tel)) { 
     $n = preg_match_all("/13\d{9}|14\d{9}|15\d{9}|18\d{9}/",$tel,$array);
     //"手机号码输入不正确目前仅支持13、14、15、18段手机号码";
     if($n==0){ 
     	 $tel_err = false; 
     }else{
       $login   = login(); //登录
  ...
  }else  $tel_num_err =  false;// echo "手机号码必须为11的数字";
  
  if($login && $balance && $tel_err && $tel_num_err){
   //发送短信
    sendSMS($tel,"尊敬的用户你好：    
   感谢使用宽带，您的账号：xx 密码xx 已2013-8-12号开通，为立即计时类型，开始时间为xxxx,结束时间为xxxx,如需更改密码请及时登录自助系统或于我们联系，联系电话****【蓝海卓越】");
   
  }else if(!$login)  return  $err[]="登录失败";
   else if(!$balance) return $err[]="余额不足";
   else if(!$tel_err) return $err[]="目前仅支持13、14、15、18段手机号码";
   else if(!$tel_num_err) return $err[]="手机号码必须为11位数字";
}  
echo getBalance();
?>
