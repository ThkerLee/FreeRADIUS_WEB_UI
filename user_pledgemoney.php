...
if($_POST){
	...
	$remark        =$_POST["remark"];
	$userID        =$_POST["userID"];
	$money  	   =$_POST["pledgemoney"];
	//更新用户剩余的押金
	//addRechargeInfo($userID,$money,1);// number 1 this subtract user's money
        $db->query("update userinfo set pledgemoney=pledgemoney-".$money." where ID='".$userID."'");
	//添加用户帐单记录，
	addUserBillInfo($userID,"f",$money,$remark);
	//添加财务记录
	addOrderRefund($userID,4,$money,$money,$orderID,$_SESSION["manager"],$remark);//number 2 ,this is reverse 
	//记录用户操作记录
	 addUserLogInfo($userID,"13",_("押金退还:").$money,getName($userID),$money);//$_SERVER['REQUEST_URI']
    echo "<script>alert('"._("退还成功!")."');</script>";	
}
?>
<table width="100%" height="500" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<table width="100%" border="0" align="center" cellpadding="2" cellspacing="0" class="title_bg2 bd">
      <tr>
        <td width="89%" class="f-bulue1"> <? echo _("押金退费")?></td>
...
<form action="?" method="post" name="myform" onSubmit="return checkReverseForm();">
        ...
        <tbody>     
		  <tr>
			...
			<input type="text" id="account" name="account" onFocus="this.className='input_on'" onBlur="this.className='input_out';ajaxInput('ajax_check.php','user_pledgemoney_account','account','accountTXT');" class="input_out"> <span id="accountTXT"></span></td>
		  </tr>
		  <tr>
		    <td align="right" class="bg"><? echo _("押金金额:")?></td>
		    <td align="left" class="bg">
				<input type="text" id="money" name="pledgemoney"   onFocus="this.className='input_on'"   onBlur="this.className='input_out';" class="input_out">			</td>
		    </tr>
		  <tr>
...
<!-----------这里是点击帮助时显示的脚本--2014.06.07----------->
 <div id="Window1" style="display:none;">
      ...
      <ul>
          <li>对已经收取押金进行退费。</li>
          <li>用户帐号：输入用户帐号，可以显示当前用户的押金余额。</li>
          <li>押金金额：需要为用户进行退费的金额。</li>
      </ul>
