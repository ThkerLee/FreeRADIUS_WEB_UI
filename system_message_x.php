...
if($_POST){  
 	 	  ...
 	 	  $sql=array(
		      ...
		      "content"=>$_POST["content"],
               ...
               if($_POST["userid"]==""  || $_POST["account"] == "" || $_POST["password"] == ""){//userid 用户名 密码 不能为空
                    echo "<script language='javascript'>alert('"._("数据不完整")."');window.location.href='system_message_x.php';</script>";
                    exit();
               }else{
	 ...
	 echo "<script language='javascript'>alert('"._("配置成功")."');window.location.href='system_message_x.php';</script>"; 
...
<form action="?action=editorderadd" method="post" name="myform"  >
 	<table width="100%" border="0" align="center" cellpadding="2" cellspacing="0" class="title_bg2 bd">
      <tr>
        ...
        <!-- <input type="radio" name="mess" id="add"  value="once"<?php if($_REQUEST['mess']=='' || $_REQUEST['act']=='add') echo "checked";?> onClick="window.location.href='?mess=add'" >  <? echo _("开户短信设置")?>-->
            <input type="radio" name="mess" id="orderadd"  value="once"<?php if($_REQUEST['mess']=='orderadd' ||$_REQUEST['mess']=='') echo "checked";?> onClick="window.location.href='?mess=orderadd'" style=" display: none;">  <? echo _("续费短信设置")?>
