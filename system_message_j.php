...
if($_POST){  
 	 	  ...
 	 	  $sql=array(
		      ...
		      "content"=>$_POST["content"],
               ...
               if($_POST["userid"]==""  || $_POST["account"] == "" || $_POST["password"] == ""){//userid 用户名 密码 不能为空
                    echo "<script language='javascript'>alert('"._("数据不完整")."');window.location.href='system_message_j.php';</script>";
                    exit();
               }else{
	 ...
	 echo "<script language='javascript'>alert('"._("配置成功")."');window.location.href='system_message_j.php';</script>"; 
      ...
      <tr>
        <td width="89%" class="f-bulue1"> 
         <!--<input type="radio" name="mess" id="add"  value="once"<? if($_REQUEST['mess']=='' || $_REQUEST['act']=='add') echo "checked";?> onClick="window.location.href='?mess=add'" >  <? echo _("开户短信设置")?>
            ...
            <input type="radio" name="mess" id="upcoming"  value="once"<?php if($_REQUEST['mess']=='upcoming'||$_REQUEST['mess']=='') echo "checked";?> onClick="window.location.href='?mess=upcoming'" style="display: none;">  <? echo _("即将到期短信设置")?>
