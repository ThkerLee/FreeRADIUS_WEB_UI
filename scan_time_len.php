	...
	//	if(!$waitOrderRs) $sql_g = false;	
		...
		if($waitOrderRs){//表示用户存在等待运行的订单，此时直接进行等待的订
		 ...
		 $enddatetime  =$waitOrderRs["enddatetime"]; 
		   if($begindatetime > "0000-00-00 00:00:00" &&  $enddatetime> "0000-00-00 00:00:00" ){
                     //  echo"非上线计时用户";
		    //非上线计时用户
...
//------------------------------------------------------------------------------------------------------------
	     ...
	     }else if($begindatetime=='0000-00-00 00:00:00' && $enddatetime =='0000-00-00 00:00:00' ){
		      //上线计时用户 
                     // echo "上线计时用户 有等待运行订单。。。更新订单的开始时间";
                     
                      //$sql_rs_eer = updateUserAttribute($userID,array("orderID"=>$orderID,"stop"=>0,"status"=>1));//原来的2014.05.28修改
          //--------------------------------2014.05.28修改上线计时等待运行订单开始---------------------------------------
			    ...
			    //更改用户属性的中的正在使用的产品编号
			    $sql_rs_eer = updateUserAttribute($userID,array("orderID"=>$waitOrderRs["orderID"],"stop"=>0,"status"=>1));
              //--------------------------------2014.05.28修改上线计时等待运行订单结束---------------------------------------    
		      if(!$sql_rs_eer) $sql_i = false;   
				  include('inc/scan_down_line.php'); 
                              //--------在t.php记录下线记录2014.03.17----------
                              $file = fopen('t.php','a');
                              $name="scan_time_len.php*1上线计时用户";
    	 ...
    	 // echo "无等待运行订单<hr>";
			...
			}else{//当余额不够,则直接结束订单 // echo "余额不以续费，停机<hr>"; //echo "当余额不够,则直接结束订单<hr>";
//-------------------------------------------------------------------------------------------------------------------------
			  if($creditline<=0){ 			  	
				   $pj=$db->select_one('projectID','userinfo','ID="'.$userID.'"');
				   if(!$pj) $sql_k = false;
				   $device =$db->select_one("device,days,status,duestatus,duepoolID","project","ID='".$pj["projectID"]."'");
			 	  ...
			 	  if($device["device"]=="sla-profile" && $device["status"]=="enable" && $device["days"]>0){ 
 			       ...
 			       if(!$sql_rs_eer) $sql_m =false;
				     ...
				     include("inc/send_notice.php"); //用户通告  
                                     /*----------------------------2014.02.28 下面添加||$device["device"]=="ma5200f" ---------2014.02.28添加碧海流控下发地址------------------------*/
			 	  }else if(($device["device"]=="mikrotik"|| $device["device"]=="ma5200f" || $device["device"]=="bhw_radius")&& $device["duestatus"]=="enable"){//ros到期后分配地址池IP
			 	  	 $sql_rs_eer = $db->query("update userrun set status=4 where orderID='".$orderID."'");
 				     if(!$sql_rs_eer) $sql_l =false;
				     $sql_rs_eer = $db->query("update orderinfo set status=4 where ID='".$orderID."'");
 			       if(!$sql_rs_eer) $sql_m =false;
				     ...
				     $sql_n = updateUserAttribute($userID,array("status"=>4,"stop"=>0));
				     //到期分配IP
				     getDuepoolIP($projectID,$userID,$UserName);  
			       ...
			       //echo "母账号账号ID：".$userID."<hr>";
 			       ...
 			       if(!$sql_rs_eer) $sql_m =false;
				     ...
				     include('inc/scan_down_line.php'); 
                           //--------在t.php记录下线记录2014.03.17----------
                              $file = fopen('t.php','a');
                              $name="scan_time_len.php*子母账号子账号停机";
                              $time=date("Y-m-d H:i:s",time())."||";
                              fwrite($file,$name.$time);
                              fclose($file);
                            //-----------------------------------------------
						...
						if(is_array($childOrderID)){ 
						 foreach($childOrderID as $child){  
								...
								include('inc/scan_down_line.php');
                                                            //--------在t.php记录下线记录2014.03.17----------
                                                                $file = fopen('t.php','a');
                                                                $name="scan_time_len.php*把子账号用户级踢下线";
                                                                $time=date("Y-m-d H:i:s",time())."||";
                                                                fwrite($file,$name.$time);
                                                                fclose($file);
                                                              //-----------------------------------------------
 ...
 if(is_array($result4)){ 
  foreach($result4 as $rs4 ){
   ...
   //if(!$rs && !is_null($rs))  $sql_aa = false; 
     ...
     if(is_array($rs)){//存在 //查看订单开始时间与当前时间对比，<=当前时间 并且 结束时间>当前时间  改变状态status=1 如果结束时间<当前时间  结束订单    ，查看余额与产品周期价格的对比进行续费操作
		  unset($orderID);
		  $orderID = $rs["orderID"]; 
		  ...
		  if($begintime > 0 &&  $endtime> 0){ 
		    if($begintime <= time() &&  $endtime  > time()){ //开始时间<=当前时间 && 结束时间>当前时间 更改status=1
		       ...
		       $sql_rs_eer =   $db->query("update userattribute set status=4 where orderID='".$orderID."'");
				   ...
				   if(!$sql_rs_eer) $sql_ee =false;
			     //1、用户余额与产品周期价格的比较
				   if((($money-$productPrice)>=0 && $productPrice>0)  || ($productPrice == "0" && $productAuto=="1")){//余额足以续费 
				      //add new order 
			   ...
			   include('inc/scan_down_line.php');
                    //--------在t.php记录下线记录2014.03.17----------
                        $file = fopen('t.php','a');
                        $name="scan_time_len.php*2上线计时用户";
                         $time=date("Y-m-d H:i:s",time())."||";
                         fwrite($file,$name.$time);
                         fclose($file);
                     //-----------------------------------------------
        ...
        //  echo "余额足以续费 <hr>";
		  ...
		  if($device["device"]=="mikrotik" && $device["duestatus"]=="enable"){//ros到期后分配地址池IP 
			 	 $sql_rs_eer = $db->query("update userrun set status=4 where orderID='".$orderID."'");
 				 if(!$sql_rs_eer) $sql_dd =false;
				 $sql_rs_eer = $db->query("update orderinfo set status=4 where ID='".$orderID."'");
 			   if(!$sql_rs_eer) $sql_ee =false;
				 //更新用户的属性条件,这是方便拨号时验证
				 $sql_ff = updateUserAttribute($userID,array("status"=>4,"stop"=>0));
		     //分配到期IP
		     getDuepoolIP($projectID,$userID,$UserName);
			}  
		 }
     ...
     $begindatetime = $waitVal["begindatetime"];
	   ...
	   $projectID = $pj["projectID"];
		 if(strtotime($enddatetime)<= time()){  
		  if($device["device"]=="mikrotik" && $device["duestatus"]=="enable"){//ros到期后分配地址池IP
			 	  $sql_rs_eer = $db->query("update userrun set status=4 where orderID='".$orderID."'");
 				  if(!$sql_rs_eer) $sql_wait_aa =false;
				  $sql_rs_eer = $db->query("update orderinfo set status=4 where ID='".$orderID."'");
 			    if(!$sql_rs_eer) $sql_wait_bb =false;
				  //更新用户的属性条件,这是方便拨号时验证
				  $sql_wait_cc = updateUserAttribute($userID,array("status"=>4,"stop"=>0));
				  //分配到期IP
				  getDuepoolIP($projectID,$userID,$UserName);
			}else{
//-----------------------------------------------------------------------------------------------
...
//第六步  扫描非ros项目用户到期后状态值userrun enddatetime <time（） and status=4 but userattribute status =1 and stop = 0 强制下线
 
$stopOrder = $db->select_all("a.userID,a.orderID,a.UserName","userattribute as a,userrun as u,project as j,userinfo as uinfo","u.orderID=a.orderID and u.userID=a.userID and uinfo.projectID=j.ID and u.userID=uinfo.ID and j.device!='mikrotik'  and u.enddatetime <'".date("Y-m-d H:i:s",time())."' and a.stop = 0 and u.status =4 and u.enddatetime !='0000-00-00-00:00:00'");  
 ...
 foreach($stopOrder as $uStop){
   ...
   if(! in_array($userID,$pjarr)){
    ...
    include('inc/scan_down_line.php');
                        //--------在t.php记录下线记录2014.03.17----------
                        $file = fopen('t.php','a');
                        $name="scan_time_len.php*扫描非ros项目";
                         $time=date("Y-m-d H:i:s",time())."||";
                         fwrite($file,$name.$time);
                         fclose($file);
                     //-----------------------------------------------
		  ...
		  if($device["device"]=="sla-profile" && $device["status"]=="enable" && $device["days"]>0){ 
 			    ...
 			    if(!$sql_rs_eer) $credit_c =false;
				  ...
				  include('inc/scan_down_line.php'); //用户下线 
                                //--------在t.php记录下线记录2014.03.17----------
                                    $file = fopen('t.php','a');
                                    $name="scan_time_len.php*阿尔卡特通告下线";
                                     $time=date("Y-m-d H:i:s",time())."||";
                                     fwrite($file,$name.$time);
                                     fclose($file);
                                 //-----------------------------------------------
	 	  }else if($device["device"]=="mikrotik" && $device["duestatus"]=="enable"){//ros到期后分配地址池IP
			 	  $sql_rs_eer = $db->query("update userrun set status=4 where orderID='".$orderID."'");
 				  if(!$sql_rs_eer) $credit_b =false;
				  $sql_rs_eer = $db->query("update orderinfo set status=4 where ID='".$orderID."'");
 			    if(!$sql_rs_eer) $credit_c =false;
        ...
        if(!$sql_rs_eer) $credit_c =false; 
				...
				//加写订单日志 
 				...
 				include('inc/scan_down_line.php');
                              //--------在t.php记录下线记录2014.03.17----------
                                    $file = fopen('t.php','a');
                                    $name="scan_time_len.php*非阿尔卡特或阿尔卡特未开启通告";
                                     $time=date("Y-m-d H:i:s",time())."||";
                                     fwrite($file,$name.$time);
                                     fclose($file);
                                 //-----------------------------------------------
						...
						if(is_array($childOrderID)){ 
						 foreach($childOrderID as $child){  
								...
								include('inc/scan_down_line.php'); 
                                                               //--------在t.php记录下线记录2014.03.17----------
                                                                $file = fopen('t.php','a');
                                                                $name="scan_time_len.php*把子账号用户级踢下线的UserName";
                                                                 $time=date("Y-m-d H:i:s",time())."||";
                                                                 fwrite($file,$name.$time);
                                                                 fclose($file);
                                                             //-----------------------------------------------
...
if(is_array($creditStatus)){ 
 ...
 if($type != "hour"){//包天数-------
  ...
  }else{//超出信用额度 ros分配到期IP 
      $pj=$db->select_one('projectID','userinfo','ID="'.$userID.'"');  
			$projectID  = $pj["projectID"];
			$device =$db->select_one("duestatus,device","project","ID='".$projectID."'"); 
      if($device["device"]=="mikrotik" && $device["duestatus"]=="enable"){//ros到期后分配地址池IP
				 //到期分配IP地址
				 getDuepoolIP($projectID,$userID,$UserName) ;  
			}
	 ...
	 if($device["device"]=="sla-profile" && $device["status"]=="enable" && $device["days"]>0){ 
	 	  ...
	 	  if(time() > $endtime ){//过了通告时间 
				  ...
				  include('inc/scan_down_line.php'); //用户下线
                                   //--------在t.php记录下线记录2014.03.17----------
                                        $file = fopen('t.php','a');
                                        $name="scan_time_len.php*过了通告时间用户下线";
                                        $time=date("Y-m-d H:i:s",time())."||";
                                        fwrite($file,$name.$time);
                                        fclose($file);
                                   //-----------------------------------------------
...
if($restoreResult)
{
	foreach($restoreResult as $restoreRs)
	{
	  ...
	  addUserLogInfo($userID,12,$_SERVER['REQUEST_URI'],$name,$surplusMoney,$operator=""); 
