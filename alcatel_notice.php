...
if(!in_array("alcatel_notice",$tableName)){
	mysql_query("
	            CREATE TABLE `alcatel_notice` (
	            `ID` INT(10) NULL DEFAULT NULL,
	            `status` VARCHAR(50) NULL DEFAULT NULL COMMENT '启用或禁用通告',
	            `addr` VARCHAR(255) NULL DEFAULT NULL COMMENT '重定向地址',
	            ...
	            `contents` VARCHAR(1024) NULL DEFAULT NULL COMMENT '内容',
	            `description` VARCHAR(1024) NULL COMMENT '描述'
              )
              COLLATE='utf8_general_ci'
              ENGINE=InnoDB;  ");
}
$rs=$db->select_one("*","alcatel_notice","0=0 order by ID desc limit 0,1");
if($_POST){ 
 if($rs){
 	 $sql=array(
 	  "addr"=>$_POST["addr"],
	 ...
	 $db->update_new("alcatel_notice","ID=1",$sql); 
 }else{
 	$sql=array(
 	  "ID"=>1,
 	  "addr"=>$_POST["addr"],
 	...
 	$db->insert_new("alcatel_notice",$sql); 
  }
   if(!empty($_POST["img"])){ 
      copy("/usr/local/usr-gui/images/".$_POST["img"], "/usr/local/alcatel-notice/alcatel_bg.jpg"); 
      exec("rm -rf /usr/local/usr-gui/images/".$_POST["img"]) ;
   }   
	 echo "<script language='javascript'>alert('"._("修改成功")."');window.location.href='alcatel_notice.php';</script>"; 
 }
?> 
<body>
...
<table width="100%" height="500" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<table width="100%" border="0" align="center" cellpadding="2" cellspacing="0" class="title_bg2 bd">
      <tr>
        <td width="89%" class="f-bulue1">贝尔到期公告配置</td>
   ...
   <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1" class="bg1" id="userinfolist">
  	<tbody>
  	  ...
  	  <tr> 
			 <td align="left" class="bg"><? echo _("重定向地址")?></td>
		   <td  align="left" class="bg">
			  <input name="addr" type="text"    id="addr"  value="<?=$rs['addr']?>" size="50"> <? echo _("请输入合法的网址以http://开头");?></td>
		  </tr> 
		  <tr> 
		  ...
		  <tr>
		    ...
		    <td align="left" class="bg">
				<textarea  name="contents" id="contents"   rows="10" style="width:400px;" ><?=$rs['contents'];?></textarea><br>
